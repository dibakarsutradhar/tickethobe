    ...
    OperatorFiltererUpgradeable
{
    ...
    function mintPhaseAllowlist(uint256 phaseIndex, uint64 quantity, bytes32[] calldata proof) external payable {
        uint64 updatedAmountMinted =
            _checkAllowlistPhaseMintConditions(msg.sender, quantity, proof, phaseIndex, msg.value);
    ...
    function mintBatch(
        ...
        for (uint256 i = 0; i < phaseLength;) {
            ...
            if (phase.merkleRoot == bytes32(0)) {
                ...
                updatedAmount =
                    _checkAllowlistPhaseMintConditions(msg.sender, quantity, proof, phaseIndex, priceForPhase);
            }
     ...
     */
    function airdropForPhase(uint256 phaseIndex, uint64[] calldata quantities, address[] calldata recipients)
        external
        onlyOwner
    {
    ...
    function _checkPhaseMintConditions(address wallet, uint256 quantity, uint256 phaseIndex, uint256 balance)
        internal
        view
        returns (uint64)
    {
